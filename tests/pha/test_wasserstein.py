...
def _flatten(diagrams: list) -> tuple:
    ...
    births = np.zeros((len(diagrams), 2), dtype=np.float64)
    return KSPersistenceTrajectory(diagrams=diagrams, essential_births=births, dt=0.02)._flatten()


def test_flatten_offsets_for_variable_size_diagrams() -> None:
    """``_flatten`` produces cumulative ``offsets`` that index each diagram's
    starting row in the concatenated array, and returns the essential
    births alongside."""
    diagrams = [
    ...
    flat, offsets, births = _flatten(diagrams)
...
def test_wasserstein_column_self_zero(rng: np.random.Generator) -> None:
    ...
    flat, offsets, births = diagrams._flatten()

    column = _wasserstein_column(flat, offsets, births, diagrams.diagrams[0], births[0])
...
def test_wasserstein_column_empty_inputs() -> None:
    ...
    no_births = np.zeros(2, dtype=np.float64)
    flat_empty, offsets_empty, births_empty = _flatten([])
    nonempty = np.array([[0.0, 1.0]], dtype=np.float64)
    assert _wasserstein_column(
        flat_empty, offsets_empty, births_empty, nonempty, no_births
    ).shape == (0,)

    flat_mix, offsets_mix, births_mix = _flatten(
        [np.zeros((0, 2)), np.array([[0.0, 1.0]], dtype=np.float64)]
    )
    column = _wasserstein_column(
        flat_mix, offsets_mix, births_mix, np.zeros((0, 2), dtype=np.float64), no_births
    )
    assert column.shape == (2,)


def test_wasserstein_column_includes_essential_classes() -> None:
    r"""The distance between the diagrams of :math:`\cos(2x)` and
    :math:`2\cos(2x)` is :math:`\sqrt{3}`: the finite pairs ``(-1, 1)`` and
    ``(-2, 2)`` match at cost 1, and the essential births each differ by 1."""
    single = KSPersistenceTrajectory([np.array([[-1.0, 1.0]])], np.array([[-1.0, 1.0]]), dt=0.02)
    double = KSPersistenceTrajectory([np.array([[-2.0, 2.0]])], np.array([[-2.0, 2.0]]), dt=0.02)
    flat, offsets, births = single._flatten()
    column = _wasserstein_column(
        flat, offsets, births, double.diagrams[0], double.essential_births[0]
    )
    # Hera's default delta = 0.01 permits 1% relative error on the finite part.
    assert column[0] == pytest.approx(np.sqrt(3.0), rel=0.01)
