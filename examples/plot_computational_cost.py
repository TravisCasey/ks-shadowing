...
Three views of detection cost. The two embedding axes are shown
independently: delays greater than 1 are used only at max order 0, so
the resolution and delay costs (panel a) are probed at max order 0 and the
derivative-order cost (panel c) at delay 1.

Panel (a): wall-clock detection time against the spatial resolution the
trajectory is loaded at, for SSA and PHA at max order 0 (the fixtures
outside resolution 2048 are ``delay = 8`` runs). SSA evaluates L2 distances in
physical space, so its
cost grows with resolution; PHA computes Wasserstein distances between
persistence diagrams, whose cost is dominated by trajectory length rather than
grid size, so its curve stays nearly flat. The full max-order-0 ``delay``
sweep at resolution 2048 is overlaid as a vertical cluster whose small extent
shows that ``delay`` has little effect on runtime; the curve passes through
the per-resolution mean.
...
Panel (c): recorded runtimes of the ``delay = 1`` derivative sweep at
resolution 2048 (max orders 3-5 exist only in that sweep), against the cost
the measured cardinalities predict. Hera's geometric auction scales
empirically as :math:`n^{1.6}` in the number of pairs per diagram, which
...
# draws the max-order-0 runs; the higher orders feed panel (c) at delay 1.
...
# %%
# Recorded runtimes of the delay-1 derivative sweep, against the cost the
...
ax_cost.set_xticks(list(DERIVATIVE_ORDERS))
