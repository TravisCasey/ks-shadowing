...
def _plot_event(  # noqa: PLR0913
    ...
    trajectory_time = np.arange(plot_start_timestep, plot_end_timestep) * dt
    relative_time = (np.arange(plot_start_timestep, plot_end_timestep) - event.start_timestep) * dt
    ...
    event_start_time = event.start_timestep * dt
    event_end_time = event.end_timestep * dt
    ...
    axes[1].axvline(duration_timesteps * dt, color="black", linestyle="--", linewidth=1.5)
    figure.colorbar(image_bottom, ax=axes[1], label="u(x,t)")

    duration_time = duration_timesteps * dt
    figure.suptitle(
...
def main() -> None:
    ...
    print(
        ...
        f"({len(trajectory) * trajectory.dt:.0f} time units)"
    )
